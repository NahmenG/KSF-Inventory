...
const DashboardView = React.memo(({ rolls, materials }) => {
  ...
  // 1. RECENT ACTIVITY LOGIC (Detecting specific states)
  const recentActivity = useMemo(() => {
    return [...rolls]
      ...
      .map(r => {
        ...
        // Logic based on status and timestamp patterns
        if (r.status === 'dispatched') {
          activity = "Dispatched";
          time = r.dispatched_at;
        } else if (r.status === 'in_stock' && r.dispatched_at) {
          // If it has a dispatch timestamp but is back in stock
          activity = "Returned";
          time = r.updated_at;
  ...
  // 2. PIE CHART LEGEND LOGIC (Weight in Tons)
  ...
  const colorChartData = useMemo(() => {
    ...
    return Object.keys(c)
      ...
      .sort((a, b) => b.weight - a.weight); // Sort highest weight to top
  }, [inStock]);
  ...
  return (
    <div className="space-y-4 pb-20 animate-in fade-in duration-500">
      ...
      {/* UPDATED RECENT LOG WITH MULTI-LINE DETAIL */}
      ...
      {/* QUALITY PIE CHART WITH TONS IN LEGEND */}
        ...
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={colorChartData} layout="vertical">
              ...
              <YAxis dataKey="name" type="category" width={60} fontSize={9} tick={{fill: '#4b5563', fontWeight: 'bold'}} />
              <Tooltip />
      ...
      {/* MATERIAL ALERT (ORDERED: POLYMER > FILLER > ADDITIVE > COLOUR) */}
