        ...
        <div className="p-2">
          {agedStock.length > 0 ? (
            <div className="space-y-1">
              {agedStock.map((r, i) => (
                <div key={i} className="p-3 flex justify-between items-center bg-white rounded-xl border border-gray-50 shadow-sm">
                  <span className="font-bold text-gray-700 text-sm">{r.product_id}</span>
                  <div className="flex items-center gap-2">
                    <span className="text-red-600 font-black text-xs">{Math.ceil((todayDate - new Date(r.created_at)) / (1000 * 60 * 60 * 24))} Days Old</span>
