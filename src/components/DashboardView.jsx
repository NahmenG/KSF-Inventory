...
import { TrendingUp, Truck, AlertCircle, ArrowUpRight } from 'lucide-react';
    ...
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      <div className="bg-white rounded-xl shadow-sm p-4 border-l-4 border-blue-600 border border-gray-100">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-green-50 p-3 rounded-lg text-center"><div className="text-[10px] text-green-700 font-bold uppercase tracking-tight">Produced</div><div className="text-2xl font-black text-green-800">{producedToday.length}</div></div>
          <div className="bg-orange-50 p-3 rounded-lg text-center"><div className="text-[10px] text-orange-700 font-bold uppercase tracking-tight">Dispatched</div><div className="text-2xl font-black text-orange-800">{dispatchedToday.length}</div></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-4 rounded-xl shadow border border-gray-100"><div className="text-gray-500 text-xs font-bold uppercase">In Stock</div><div className="text-2xl font-black text-blue-600">{inStock.length} <span className="text-xs font-normal text-gray-400">Rolls</span></div></div>
        <div className="bg-white p-4 rounded-xl shadow border border-gray-100"><div className="text-gray-500 text-xs font-bold uppercase">Total weight</div><div className="text-2xl font-black text-green-600">{formatCurrency(stats.stockWt)} <span className="text-xs font-normal text-gray-400">kg</span></div></div>
      </div>
          ...
          <div className="space-y-2">
            {lowMaterials.map(m => (
              <div key={m.id} className="flex justify-between items-center bg-white p-2 rounded border border-red-100 text-sm shadow-sm">
                ...
                <span className="font-bold text-red-600">{m.stock_quantity} kg <span className="text-[10px] text-gray-400 font-normal">/ {m.min_level} kg</span></span>
              </div>
            ))}
          </div>
        </div>
