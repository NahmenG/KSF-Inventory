...
const StockView = React.memo(({ rolls, onPrint, onSelectRoll }) => {
  ...
  // Each field works independently to narrow down thousands of rolls
  ...
  // Handles real-time filtering for 1000+ records
  ...
  return (
    <div className="space-y-4 pb-20 animate-in fade-in duration-500">
      ...
      <div className="sticky top-0 z-30 bg-slate-50/95 backdrop-blur-md pt-2 space-y-2">
          ...
          <div className="flex justify-between items-center mb-4">
            ...
            {(filters.customer || filters.gsm || filters.width || filters.color) && (
              <button 
                ...
                className="text-[10px] font-black text-red-500 flex items-center gap-1 bg-red-50 px-2 py-1 rounded-lg hover:bg-red-100 transition-colors"
              >
                <X size={12} /> Reset
              </button>
            )}
        ...
        {/* SMALLER ON MOBILE (p-3, text-xl) | LARGER ON DESKTOP (md:p-6, md:text-4xl) */}
        <div className="bg-gray-900 text-white p-3 md:p-6 rounded-2xl flex justify-between items-center shadow-2xl border border-gray-800 transition-all">
          <div className="flex flex-col">
            <span className="text-[8px] md:text-[10px] text-gray-500 font-black uppercase tracking-[0.2em] mb-1">Available Rolls</span>
            <span className="text-xl md:text-4xl font-black">
              {filtered.length} <span className="text-[10px] md:text-xs font-normal opacity-40">Qty</span>
            </span>
          </div>
          <div className="text-right flex flex-col">
            <span className="text-[8px] md:text-[10px] text-gray-500 font-black uppercase tracking-[0.2em] mb-1">Stock Net Weight</span>
