...
const StockView = React.memo(({ rolls, onPrint, onSelectRoll }) => {
  const [filters, setFilters] = useState({ customer: '', gsm: '', width: '', color: '' });
  ...
  const handleExport = () => {
    const data = filtered.map(r => ({
      "Roll ID": r.product_id, "Buyer": r.customer_name, "GSM": r.gsm, "Width": r.width_inches, "Color": r.color, "Net Kg": r.net_weight, "Gross Kg": r.gross_weight, "Date": new Date(r.created_at).toLocaleString()
    }));
      ...
      <div className="sticky top-0 z-30 bg-slate-50/95 backdrop-blur-md pt-2 space-y-2">
        <div className="bg-white p-4 rounded-2xl shadow-md border border-gray-100">
          <div className="flex justify-between items-center mb-4">
            ...
            {(filters.customer || filters.gsm || filters.width || filters.color) && (
              <button onClick={() => setFilters({customer: '', gsm: '', width: '', color: ''})} className="text-[10px] font-black text-red-500 flex items-center gap-1 bg-red-50 px-2 py-1 rounded-lg">
                <X size={12} /> Reset
              </button>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="flex flex-col gap-1">
              ...
              <input className="border border-gray-100 p-2.5 rounded-xl text-xs font-bold bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100" placeholder="Ex: Reliance" value={filters.customer} onChange={e => setFilters({...filters, customer: e.target.value})} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[9px] font-black text-gray-400 uppercase ml-1">GSM</label>
              <input type="number" className="border border-gray-100 p-2.5 rounded-xl text-xs font-bold bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100" placeholder="Ex: 60" value={filters.gsm} onChange={e => setFilters({...filters, gsm: e.target.value})} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[9px] font-black text-gray-400 uppercase ml-1">Size (in)</label>
              <input type="number" className="border border-gray-100 p-2.5 rounded-xl text-xs font-bold bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100" placeholder="Ex: 42" value={filters.width} onChange={e => setFilters({...filters, width: e.target.value})} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[9px] font-black text-gray-400 uppercase ml-1">Color</label>
              <input className="border border-gray-100 p-2.5 rounded-xl text-xs font-bold bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100" placeholder="Ex: White" value={filters.color} onChange={e => setFilters({...filters, color: e.target.value})} />
            </div>
          </div>
          ...
          <div className="flex gap-2 mt-4 pt-4 border-t border-gray-50">
            ...
            <button onClick={handleExport} className="flex-1 bg-green-600 text-white rounded-xl font-black text-xs flex items-center justify-center gap-2 active:scale-95 transition-all">
              <Download size={14}/> Export XLS
            </button>
          </div>
        </div>

        {/* RESPONSIVE BLACK BAR: Shorter on Mobile | Taller on Desktop */}
        <div className="bg-gray-900 text-white p-3 md:p-6 rounded-2xl flex justify-between items-center shadow-2xl border border-gray-800 transition-all">
          ...
          <div className="text-right flex flex-col">
            <span className="text-[8px] md:text-[10px] text-gray-500 font-black uppercase tracking-[0.2em] mb-1">Net Weight</span>
        ...
        {filtered.map(r => (
          <div key={r.id} onClick={() => onSelectRoll(r)} className="bg-white p-4 rounded-2xl border border-gray-100 flex justify-between items-center active:scale-[0.98] transition-all shadow-sm hover:border-blue-200 cursor-pointer group">
            ...
            <div className="text-right flex flex-col items-end min-w-[100px]">
              ...
              <button onClick={(e) => { e.stopPropagation(); onPrint(r); }} className="text-blue-500 mt-3 p-2.5 bg-blue-50 group-hover:bg-blue-600 group-hover:text-white rounded-xl transition-all shadow-sm">
                <Printer size={20}/>
              </button>
            </div>
          </div>
