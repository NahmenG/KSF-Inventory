...
const StockView = React.memo(({ rolls, onPrint, onSelectRoll }) => {
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, `KSF_Inventory_Export_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  return (
    <div className="space-y-4 pb-20">
      ...
      <div className="sticky top-0 z-30 bg-slate-50/90 backdrop-blur-md pt-2 space-y-2">
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="flex flex-col gap-1">
              ...
              <input className="border border-gray-100 p-2.5 rounded-xl text-xs font-bold bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100 transition-all" placeholder="Type name..." value={filters.customer} onChange={e => setFilters({...filters, customer: e.target.value})} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[9px] font-black text-gray-400 uppercase ml-1">GSM</label>
              <input type="number" className="border border-gray-100 p-2.5 rounded-xl text-xs font-bold bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100 transition-all" placeholder="e.g. 60" value={filters.gsm} onChange={e => setFilters({...filters, gsm: e.target.value})} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[9px] font-black text-gray-400 uppercase ml-1">Size (in)</label>
              <input type="number" className="border border-gray-100 p-2.5 rounded-xl text-xs font-bold bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100 transition-all" placeholder="e.g. 40" value={filters.width} onChange={e => setFilters({...filters, width: e.target.value})} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-[9px] font-black text-gray-400 uppercase ml-1">Color</label>
              <input className="border border-gray-100 p-2.5 rounded-xl text-xs font-bold bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100 transition-all" placeholder="White/Red..." value={filters.color} onChange={e => setFilters({...filters, color: e.target.value})} />
            </div>
          </div>

          <div className="flex gap-2 mt-4 pt-4 border-t border-gray-50">
            <button onClick={() => setSort(sort === 'newest' ? 'oldest' : 'newest')} className="flex-1 py-2.5 border border-gray-100 rounded-xl bg-white text-xs font-black text-gray-600 flex items-center justify-center gap-2 active:scale-95 transition-all">
              {sort === 'newest' ? <><ArrowDown size={14} className="text-blue-500"/> Newest</> : <><ArrowUp size={14} className="text-blue-500"/> Oldest</>}
            </button>
            <button onClick={handleExport} className="flex-1 bg-green-600 text-white rounded-xl font-black text-xs flex items-center justify-center gap-2 active:scale-95 transition-all shadow-lg shadow-green-100">
              <Download size={14}/> Export XLS
            </button>
          </div>
        </div>

        {/* SECTION 2: RESPONSIVE BLACK SUMMATION BAR */}
        <div className="bg-gray-900 text-white p-3 md:p-6 rounded-2xl flex justify-between items-center shadow-2xl border border-gray-800 transition-all">
          <div className="flex flex-col">
            <span className="text-[8px] md:text-[10px] text-gray-500 font-black uppercase tracking-[0.2em] mb-1">Result Count</span>
          ...
          <div className="text-right flex flex-col">
            <span className="text-[8px] md:text-[10px] text-gray-500 font-black uppercase tracking-[0.2em] mb-1">Net Total</span>
      ...
      {/* SECTION 3: ROLL CARDS LIST */}
      <div className="space-y-2 mt-4">
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-gray-400 bg-white rounded-2xl border-2 border-dashed border-gray-100 font-bold italic">
          ...
          filtered.map(r => (
            <div key={r.id} onClick={() => onSelectRoll(r)} className="bg-white p-4 rounded-2xl border border-gray-100 flex justify-between items-center active:scale-[0.98] transition-all shadow-sm hover:border-blue-200 cursor-pointer group">
              <div>
                ...
                <div className="text-[10px] text-gray-400 uppercase font-black mt-1 flex gap-2">
                  <span className="text-gray-500">{r.quality}</span>
                  <span className="text-blue-500">{r.color}</span>
                  <span className="text-orange-600">{r.gsm} GSM</span>
                  <span className="text-green-600">{r.width_inches}"</span>
                </div>
              </div>
              <div className="text-right flex flex-col items-end">
                ...
                <button onClick={(e) => { e.stopPropagation(); onPrint(r); }} className="text-blue-500 mt-3 p-2 bg-blue-50 group-hover:bg-blue-600 group-hover:text-white rounded-xl transition-all">
                  <Printer size={20}/>
                </button>
              </div>
            </div>
