...
import { Search, Printer, Download, ArrowDown, ArrowUp, Filter } from 'lucide-react';
...
const StockView = React.memo(({ rolls, onPrint, onSelectRoll }) => {
  ...
  const handleExport = () => {
    const data = filtered.map(r => ({ "ID": r.product_id, "Buyer": r.customer_name, "Quality": r.quality, "Color": r.color, "GSM": r.gsm, "Net Kg": r.net_weight, "Date": new Date(r.created_at).toLocaleDateString() }));
      ...
      <div className="bg-white p-3 rounded-xl shadow-sm border sticky top-0 z-10 space-y-2">
        <div className="flex gap-2">
          <div className="flex-1 flex gap-2 border p-2 rounded-lg bg-gray-50 items-center focus-within:ring-2 focus-within:ring-blue-100 transition-all">
            ...
            <input className="w-full outline-none text-sm bg-transparent" placeholder="Search rolls..." value={query} onChange={e => setQuery(e.target.value)} />
          </div>
          <button onClick={() => setSort(sort === 'newest' ? 'oldest' : 'newest')} className="p-2 border rounded-lg hover:bg-gray-50 transition-colors">{sort === 'newest' ? <ArrowDown size={20}/> : <ArrowUp size={20}/>}</button>
        ...
        <div className="bg-gray-900 text-white p-2 rounded-lg flex justify-between text-[10px] font-bold px-4 uppercase tracking-tighter">
          <span>Displaying {filtered.length} Rolls</span>
        ...
        {filtered.map(r => (
          <div key={r.id} onClick={() => onSelectRoll(r)} className="bg-white p-4 rounded-xl border border-gray-100 flex justify-between items-center active:scale-95 transition-all shadow-sm hover:border-blue-200">
            ...
            <div className="text-right flex flex-col items-end">
              ...
              <button onClick={(e) => { e.stopPropagation(); onPrint(r); }} className="text-blue-500 mt-2 p-2 hover:bg-blue-50 rounded-full transition-colors"><Printer size={20}/></button>
            </div>
          </div>
        ))}
      </div>
