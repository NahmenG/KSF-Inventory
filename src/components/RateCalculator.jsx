...
import { Calculator, TrendingUp, History, ShieldCheck, ChevronRight, IndianRupee } from 'lucide-react';
...
export default function RateCalculator() {
  ...
  // 1. DEFINING RECIPES (Based on your 100kg Batch Strategy)
  const recipes = {
  ...
  useEffect(() => {
    const fetchRates = async () => {
      ...
      if (!error && data) {
        // Get only the most recent rate for each material
        const latest = [];
        const seen = new Set();
        data.forEach(item => {
  ...
  const pricing = useMemo(() => {
    ...
    Object.keys(recipes).forEach(qName => {
      ...
      results[qName] = costPerKg;
    });
    return results;
  }, [marketRates]);

  const currentRatePerKg = pricing[selectedQuality] || 0;
  const totalBill = (parseFloat(customWeight) || 0) * currentRatePerKg;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {Object.keys(recipes).map(q => (
          <button 
            ...
            className={`p-4 rounded-[2rem] border transition-all text-left ${selectedQuality === q ? 'bg-blue-600 border-blue-600 shadow-lg shadow-blue-100' : 'bg-white border-slate-100 hover:border-blue-200'}`}
          >
            <div className={`text-[8px] font-black uppercase tracking-widest mb-1 ${selectedQuality === q ? 'text-blue-100' : 'text-slate-400'}`}>{q}</div>
            <div className={`text-lg font-black ${selectedQuality === q ? 'text-white' : 'text-slate-900'}`}>₹{pricing[q]?.toFixed(2)}</div>
      ...
      <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col md:flex-row gap-6 items-center">
        <div className="flex-1 w-full">
          <div className="flex items-center gap-2 mb-4">
            <div className="p-2 bg-green-50 rounded-xl text-green-600"><Calculator size={18}/></div>
            <span className="text-xs font-black uppercase tracking-tighter text-slate-500">Fast Quote: {selectedQuality}</span>
          </div>
          <div className="relative">
            ...
            />
            <div className="absolute right-5 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-400 uppercase">Weight KG</div>
          </div>
        </div>

        <div className="w-full md:w-64 bg-slate-900 p-6 rounded-[2rem] text-center shadow-xl">
          ...
          <div className="text-[9px] font-bold text-slate-500 uppercase mt-2">Incl. GST 12% Extra</div>
        </div>
      </div>

      {/* SECTION 3: RATE HISTORY TOGGLE */}
      <div className="bg-slate-50 p-4 rounded-2xl flex items-center justify-between border border-slate-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] font-black uppercase text-slate-400 tracking-tight">Market Status</div>
            <div className="text-[11px] font-bold text-slate-700">PP Virgin: ₹{marketRates.find(r => r.material_name === 'PP Virgin')?.rate || '0'} /kg</div>
          </div>
        </div>
        <button className="px-4 py-2 bg-white rounded-xl text-[10px] font-black text-blue-600 uppercase border border-slate-200 shadow-sm flex items-center gap-2">
          Update Rates <ChevronRight size={12}/>
        </button>
      </div>
    </div>
  );
