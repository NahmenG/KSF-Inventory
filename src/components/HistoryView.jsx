    ...
    <div className="space-y-4 pb-20 animate-in fade-in duration-500">
      <div className="sticky top-0 z-30 bg-slate-50/95 backdrop-blur-md pt-2 space-y-2 pb-2">
        <div className="bg-white px-4 py-3 rounded-2xl shadow-md border border-gray-100 relative">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-4 gap-1.5 items-center">
              ...
              <button onClick={() => onFetchRange(filters.startDate, filters.endDate)} className="bg-blue-600 text-white rounded-xl font-black text-[9px] h-full py-2 flex items-center justify-center gap-1 shadow-md active:scale-95 transition-all"><RefreshCw size={12} /> FETCH RANGE</button>
              <button onClick={handleExport} className="bg-green-600 text-white rounded-xl font-black text-[9px] h-full py-2 flex items-center justify-center gap-1 shadow-md active:scale-95 transition-all"><Download size={12} /> EXCEL</button>
            </div>
          </div>
        </div>
        <div className="bg-gray-900 text-white p-3 md:p-4 rounded-2xl flex justify-between items-center shadow-2xl border border-gray-800">
          <div className="flex flex-col"><span className="text-[8px] md:text-[9px] text-gray-500 font-black uppercase tracking-[0.2em] mb-0.5">{activeRange === '30days' ? 'Record for last 30 days' : 'Record for selected range'}</span><span className="text-xl md:text-2xl font-black text-orange-400">{filtered.length} <span className="text-[10px] md:text-xs font-normal opacity-40 text-white ml-1">Rolls</span></span></div>
