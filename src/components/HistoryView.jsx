    ...
    <div className="space-y-4 pb-20 animate-in fade-in duration-500">
      <div className="sticky top-0 z-30 bg-slate-50/95 backdrop-blur-md pt-2 space-y-2 pb-2">
        ...
        <div className="bg-gray-900 text-white p-3 rounded-2xl flex justify-between items-center shadow-2xl">
          <div className="flex flex-col"><span className="text-[8px] text-gray-500 font-black uppercase tracking-[0.2em]">{activeRange === '30days' ? 'Record for last 30 days' : 'Record for selected range'}</span><span className="text-xl font-black text-orange-400">{filtered.length} Rolls</span></div>
          <div className="text-right flex flex-col"><span className="text-[8px] text-gray-500 font-black uppercase tracking-[0.2em]">Total weight</span><span className="text-xl font-black text-green-400">{filtered.reduce((s,r)=>s+(parseFloat(r.net_weight)||0),0).toFixed(1)} kg</span></div>
        </div>
      </div>
      <div className="space-y-2 px-1">
        {filtered.map(r => (
          <div key={r.id} onClick={() => onSelectRoll(r)} className="bg-white p-4 rounded-2xl border border-gray-100 flex justify-between items-center active:scale-[0.98] cursor-pointer">
            <div className="flex-1"><div className="font-black text-blue-600 text-lg flex items-center gap-2">{r.product_id} <span className={`text-[7px] font-black uppercase px-2 py-0.5 rounded-full ${r.status === 'in_stock' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>{r.status === 'in_stock' ? 'In Stock' : 'Dispatched'}</span></div><div className="text-sm font-black text-gray-800 mt-1">{r.customer_name || 'Generic Stock'}</div><div className="text-[10px] text-gray-400 font-black mt-1 flex gap-2"><span className="bg-slate-50 px-1.5 rounded">{r.quality}</span><span>{r.color}</span><span>{r.gsm} GSM</span><span>{r.width_inches}"</span></div></div>
            <div className="text-right flex flex-col items-end min-w-[100px]"><div className="font-black text-2xl text-gray-900 leading-none">{r.net_weight} <span className="text-[10px] text-gray-400">kg</span></div><div className="text-[10px] font-bold text-gray-400 flex items-center gap-1 mt-2"><Clock size={10} /> {new Date(r.dispatched_at || r.created_at).toLocaleDateString()}</div></div>
          </div>
        ))}
      </div>
    </div>
