...
const LabelPrint = ({ data, onClose }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Restored your exact dimensions: 2.4in x 3.9in
        ...
        <div className="flex-1 overflow-auto bg-gray-100 p-4 flex justify-center">
          ...
          <div ref={labelRef} className="flex flex-col justify-between items-center text-center bg-white shadow-xl relative" style={{ width: '2.4in', height: '3.9in', padding: '0.1in', boxSizing: 'border-box' }}>
            ...
            <div className="w-full border-y-2 border-black py-2 my-1 flex justify-between items-end px-1">
              ...
              <div className="text-right"><span className="text-[10px] uppercase font-bold block text-gray-400">Net Weight</span><span className="text-4xl font-black leading-none">{data.net_weight}<span className="text-lg">kg</span></span></div>
            </div>
            ...
            <div className="w-full flex flex-col items-center overflow-hidden pb-6">
              ...
              {showDate && <div className="text-[8px] text-gray-500 mt-1 leading-none">{new Date(data.created_at || new Date()).toLocaleString()}</div>}
            </div>
          </div>
        </div>

        <div className="p-4 bg-gray-50 flex gap-2 border-t">
          <button onClick={handleDownloadPDF} disabled={isGenerating} className="flex-1 bg-blue-600 text-white py-3 rounded-lg font-bold shadow flex justify-center gap-2 items-center active:scale-95">
            {isGenerating ? 'Saving...' : <><Printer size={18} /> Save PDF</>}
          </button>
          <button onClick={onClose} className="flex-1 bg-white border text-gray-700 py-3 rounded-lg font-bold active:scale-95">Close</button>
        </div>
      </div>
    </div>
  );
