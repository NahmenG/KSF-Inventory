...
// REMOVED 'Plus' icon entirely. Using safe alternatives.
import { 
  Package, Truck, Layers, LogOut, Printer, Search, 
  Download, Database, AlertCircle, Calendar, Clock, 
  Pencil, Trash2, X, Camera, Smartphone, Activity, Eye, FileText, Filter, RotateCcw, CheckCircle, AlignLeft, 
  Settings, Minus, ZoomIn, ChevronUp, ChevronDown
} from 'lucide-react';
    ...
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-red-50 p-6 text-center">
          ...
          <p className="mb-6 text-gray-700">A safety check stopped the app from crashing completely.</p>
          <button 
...
const generateChallan = (rolls, details) => {
    try {
        ...
        doc.autoTable({ startY: 75, head: [['#', 'Roll ID', 'Quality', 'Color', 'Size', 'GSM', 'Weight']], body: tableData, theme: 'grid' });
...
const Header = React.memo(({ user, isGuest, deviceName, onLogout, setTab, activeTab, onEditDeviceName }) => (
  <header className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm safe-area-inset-top">
    ...
    <nav className="flex gap-1 px-2 overflow-x-auto no-scrollbar max-w-7xl mx-auto border-t border-gray-100 md:border-none">
      {[{ id: 'dashboard', icon: Database, label: 'Dash' }, !isGuest && { id: 'entry', icon: FileText, label: 'New' }, { id: 'stock', icon: Package, label: 'Stock' }, { id: 'dispatch', icon: Truck, label: 'Dispatch' }, { id: 'history', icon: Calendar, label: 'History' }, { id: 'materials', icon: Layers, label: 'Materials' }].filter(Boolean).map(tab => (
...
const LabelPrint = ({ data, onClose }) => {
  const savedDesign = safeJSONParse('ksf_label_design_v12', {});
  const savedLabels = safeJSONParse('ksf_label_labels_v12', {});
  ...
  const [content, setContent] = useState({
      ...
      widthLabel: savedLabels.widthLabel || 'Width:', widthVal: data?.width_inches ? `${data.width_inches}"` : '',
      lengthLabel: savedLabels.lengthLabel || 'Length:', lengthVal: data?.length_meters ? `${data.length_meters}m` : '',
      netLabel: savedLabels.netLabel || 'Net Wt:', netVal: data?.net_weight ? `${data.net_weight}kg` : '',
      grossLabel: savedLabels.grossLabel || 'Gross Wt:', grossVal: data?.gross_weight ? `${data.gross_weight}kg` : ''
  });
  const canvasRef = useRef(null);
  useEffect(() => {
      localStorage.setItem('ksf_label_design_v12', JSON.stringify({ showLogo, labelSize, baseFontSize, logoSize, barcodeScale }));
      const labelsOnly = { qualityLabel: content.qualityLabel, gsmLabel: content.gsmLabel, colorLabel: content.colorLabel, widthLabel: content.widthLabel, lengthLabel: content.lengthLabel, netLabel: content.netLabel, grossLabel: content.grossLabel };
      localStorage.setItem('ksf_label_labels_v12', JSON.stringify(labelsOnly));
...
const NewProductView = ({ formData, setFormData, onSubmit }) => (
    <div className="bg-white p-4 rounded-lg shadow border mt-2">
      {/* NO PLUS ICON HERE */}
      <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-gray-800 border-b pb-2"><FileText size={20} className="text-blue-600"/> New Roll Entry</h2>
...
const DispatchView = ({ rolls, isGuest, deviceName, onDispatch, onUpdateRoll }) => {
    ...
    const [sessionList, setSessionList] = useState(() => safeJSONParse('ksf_dispatch_list_v13', []));
    const [customerName, setCustomerName] = useState(() => localStorage.getItem('ksf_dispatch_customer_v13') || '');
    const [vehicleNo, setVehicleNo] = useState(() => localStorage.getItem('ksf_dispatch_vehicle_v13') || '');
    useEffect(() => {
        localStorage.setItem('ksf_dispatch_list_v13', JSON.stringify(sessionList));
        localStorage.setItem('ksf_dispatch_customer_v13', customerName);
        localStorage.setItem('ksf_dispatch_vehicle_v13', vehicleNo);
