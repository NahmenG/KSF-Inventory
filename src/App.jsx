...
// --- LABEL PRINT COMPONENT (Unchanged) ---
...
const DispatchView = ({ rolls, isGuest, deviceName, onDispatch, onUpdateRoll }) => {
    ...
    const handleRemoveFromList = async (itemToRemove) => {
        if(!window.confirm(`Remove ${itemToRemove.product_id}? This will return it to Stock.`)) return;
        try {
    ...
    const handlePrintChallan = () => {
        ...
        if(generateChallan(sessionList, { buyer: customerName, vehicle: vehicleNo, device: deviceName })) {
             if(window.confirm("Gate Pass Generated. Start new batch?")) { 
    ...
    return (
        <div className="max-w-xl mx-auto space-y-4 pb-20">
            ...
            <div className="bg-blue-600 p-4 rounded-xl text-white shadow-lg shadow-blue-200"><h3 className="font-bold text-sm opacity-80 uppercase mb-2 flex items-center gap-2"><FileText size={16}/> Dispatch Manifest</h3><input className="w-full bg-blue-700/50 border border-blue-500 rounded p-2 text-white placeholder-blue-300 font-bold mb-2 outline-none focus:bg-blue-700" placeholder="Customer Name" value={customerName} onChange={e => setCustomerName(e.target.value)} /><input className="w-full bg-blue-700/50 border border-blue-500 rounded p-2 text-white placeholder-blue-300 text-sm outline-none focus:bg-blue-700" placeholder="Vehicle No" value={vehicleNo} onChange={e => setVehicleNo(e.target.value)} /><div className="flex justify-between items-end mt-3 border-t border-blue-500 pt-2"><div><div className="text-xs opacity-70">Date</div><div className="font-bold text-sm">{new Date().toLocaleDateString()}</div></div><div className="text-right"><div className="text-xs opacity-70">Total List Weight</div><div className="font-black text-2xl">{totalWeight.toLocaleString()} <span className="text-sm font-normal">kg</span></div></div></div></div>
