...
// REMOVED 'Plus' to prevent crashes. Using other stable icons.
import { 
    ...
    <nav className="flex gap-1 px-2 overflow-x-auto no-scrollbar max-w-7xl mx-auto border-t border-gray-100 md:border-none">
      {[{ id: 'dashboard', icon: Database, label: 'Dash' }, !isGuest && { id: 'entry', icon: Package, label: 'New' }, { id: 'stock', icon: Package, label: 'Stock' }, { id: 'dispatch', icon: Truck, label: 'Dispatch' }, { id: 'history', icon: Calendar, label: 'History' }, { id: 'materials', icon: Layers, label: 'Materials' }].filter(Boolean).map(tab => (
        <button key={tab.id} onClick={() => setTab(tab.id)} className={`flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2 px-3 py-3 md:py-3 rounded-t-lg text-[10px] md:text-sm font-medium transition-all min-w-[65px] flex-1 md:flex-none border-b-2 ${activeTab === tab.id ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`}>
          {tab.id === 'entry' ? <span className="text-lg font-bold leading-none">+</span> : <tab.icon size={18} className={activeTab === tab.id ? "text-blue-600" : "text-gray-400"} />}
          <span>{tab.label}</span>
        </button>
      ))}
    </nav>
...
const NewProductView = ({ formData, setFormData, onSubmit }) => (
    <div className="bg-white p-4 rounded-lg shadow border mt-2">
      {/* Replaced Icon with Text to Prevent Crash */}
      <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-gray-800 border-b pb-2"><span className="text-blue-600 text-xl font-bold">+</span> New Roll Entry</h2>
