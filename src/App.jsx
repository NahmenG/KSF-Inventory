    ...
    <div className="space-y-6 pb-20">
      <div className="bg-white rounded-xl shadow p-4 border-l-4 border-blue-600">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-green-50 p-3 rounded-lg"><div className="text-xs text-green-700 font-bold uppercase tracking-tight">Produced</div><div className="text-2xl font-bold text-green-800">{producedToday}</div></div>
          <div className="bg-orange-50 p-3 rounded-lg"><div className="text-xs text-orange-700 font-bold uppercase tracking-tight">Dispatched</div><div className="text-2xl font-bold text-orange-800">{dispatchedToday}</div></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white p-4 rounded-xl shadow border border-gray-100"><div className="text-gray-500 text-xs font-bold uppercase">Stock Weight</div><div className="text-2xl font-bold text-green-600">{formatCurrency(totalWeight)} <span className="text-sm text-gray-400 font-normal">kg</span></div></div>
      </div>
      ...
      <div className="sticky top-16 z-20 bg-slate-50 pt-3 pb-2">
        <div className="bg-white p-3 rounded-xl shadow-sm flex flex-col gap-3 border border-gray-100">
          ...
          <div className="bg-gray-900 text-white p-3 rounded flex justify-between items-center text-sm">
            <div><div className="text-gray-400 text-xs uppercase tracking-tighter">Found</div><div className="font-bold">{filtered.length} Rolls</div></div>
            <div className="text-right"><div className="text-gray-400 text-xs uppercase tracking-tighter">Total Weight</div><div className="font-bold text-lg text-yellow-400">{formatCurrency(totalFilteredWeight)} kg</div></div>
          </div>
          {showFilters && (
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-sm max-h-[90vh] overflow-y-auto shadow-2xl border">
            ...
            <div className="bg-blue-50 p-3 rounded text-center mb-4 border border-blue-100">
              <div className="text-xs font-bold text-blue-400 uppercase tracking-tighter">Roll ID</div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              <div><label className="text-[10px] font-bold text-gray-500 uppercase">Quality</label><div className="bg-white border p-2 rounded text-sm font-bold text-gray-700">{reviewData.quality}</div></div>
              <div><label className="text-[10px] font-bold text-gray-500 uppercase">Weight</label><div className="bg-white border p-2 rounded text-sm font-bold text-green-700">{reviewData.net_weight} kg</div></div>
            </div>
              ...
              <div key={i} className="p-3 border-b flex justify-between items-center last:border-0 hover:bg-gray-50">
                <div>
                  ...
                  <div className="text-[10px] text-gray-400 uppercase tracking-tighter">
                    {item.quality} • {item.net_weight}kg
                  </div>
                </div>
                <button onClick={() => handleRemoveFromManifest(i, item)} className="text-red-500 p-2"><Trash2 size={18}/></button>
              </div>
            ))}
          </div>
...
const MaterialsView = React.memo(({ materials, isGuest, onUpdate, onAdd, onEdit, onDelete }) => {
  ...
  const handleDelete = (id) => { if (confirm("Delete this?")) onDelete(id); };
    ...
    return (
      <div className="h-[100dvh] flex items-center justify-center bg-slate-50 p-6">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-sm w-full text-center border border-gray-100 tracking-tighter">
          ...
          <h1 className="text-2xl font-black mb-2 text-gray-900 tracking-tighter">KSF Inventory</h1>
  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 font-sans pt-16 pb-20">
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6 animate-in fade-in duration-500">
        {loading && activeTab !== 'dashboard' ? (<div className="flex justify-center p-12 text-gray-400 italic">Accessing database...</div>) : (
          <>
