...
// --- HELPER: GENERATE PDF CHALLAN (FIXED) ---
const generateChallan = (rolls, details) => {
    try {
        ...
        // Footer Calculations (Safe Position Check)
...
const NewProductView = ({ formData, setFormData, onSubmit }) => {
  return (
    <div className="max-w-xl mx-auto bg-white p-5 rounded-xl shadow-sm border border-gray-100 mt-2">
      <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-gray-800 border-b pb-2"><PlusIcon size={20} className="text-blue-600"/> New Roll Entry</h2>
...
const AnalyticsDashboard = ({ rolls }) => {
    ...
    const productionData = last7Days.map(date => { const count = (rolls || []).filter(r => r.created_at && r.created_at.startsWith(date)).length; return { name: date.slice(5), rolls: count }; });
