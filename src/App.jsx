...
const Header = React.memo(({ user, isGuest, deviceName, onLogout, setTab, activeTab, onEditDeviceName }) => (
  <header className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm safe-area-inset-top">
    ...
    <nav className="flex gap-1 px-2 overflow-x-auto no-scrollbar max-w-7xl mx-auto border-t border-gray-100 md:border-none">
      {[{ id: 'dashboard', icon: Database, label: 'Dash' }, !isGuest && { id: 'entry', icon: Package, label: 'New' }, { id: 'stock', icon: Package, label: 'Stock' }, { id: 'dispatch', icon: Truck, label: 'Dispatch' }, { id: 'history', icon: Calendar, label: 'History' }, { id: 'materials', icon: Layers, label: 'Materials' }].filter(Boolean).map(tab => (
...
const BarcodeScanner = ({ onScan, onClose }) => {
    useEffect(() => {
        ...
        html5QrCode.start({ facingMode: "environment" }, config, onScan, () => {}).catch(console.error);
    ...
    return (
        <div className="fixed inset-0 bg-black/90 z-[100] flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-sm bg-white rounded-xl overflow-hidden p-4">
                 ...
                 <button onClick={onClose} className="w-full mt-4 bg-red-100 text-red-600 py-3 rounded-lg font-bold">Close</button>
            </div>
        </div>
    );
};
...
const LabelPrint = ({ data, onClose }) => {
  ...
  useEffect(() => {
      try {
          const savedDesign = JSON.parse(localStorage.getItem('ksf_label_design_v5') || '{}');
          if (savedDesign.labelSize) setLabelSize(savedDesign.labelSize);
          if (savedDesign.baseFontSize) setBaseFontSize(savedDesign.baseFontSize);
      } catch (e) {}
  }, []);

  useEffect(() => {
      localStorage.setItem('ksf_label_design_v5', JSON.stringify({ showLogo, labelSize, baseFontSize, logoSize, barcodeScale }));
  }, [showLogo, labelSize, baseFontSize, logoSize, barcodeScale]);
...
const NewProductView = ({ formData, setFormData, onSubmit }) => {
  return (
    <div className="max-w-xl mx-auto bg-white p-5 rounded-xl shadow-sm border border-gray-100 mt-2">
      <h2 className="text-lg font-bold mb-4 flex items-center gap-2 text-gray-800 border-b pb-2"><Pencil size={20} className="text-blue-600"/> New Roll Entry</h2>
...
const DispatchView = ({ rolls, isGuest, deviceName, onDispatch, onUpdateRoll }) => {
    ...
    // Version 5 Key to force reset
    const [sessionList, setSessionList] = useState(() => {
        try { return JSON.parse(localStorage.getItem('ksf_dispatch_list_v5') || '[]'); } catch { return []; }
    });
    ...
    useEffect(() => {
        localStorage.setItem('ksf_dispatch_list_v5', JSON.stringify(sessionList));
