...
import { 
  ...
  Pencil, Trash2, X, Camera, Smartphone, Activity, Eye, FileText, Filter, RotateCcw, CheckCircle, AlignLeft
} from 'lucide-react';
            ...
            {isGuest ? (<div className="flex items-center gap-1 text-xs font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded-full"><Eye size={12}/> View Only</div>) : (
                <>
                    <div className="flex items-center justify-end gap-1 group cursor-pointer" onClick={onEditDeviceName} title="Edit Device Name">
                        ...
                        <Pencil size={10} className="text-gray-400 group-hover:text-blue-600"/>
                    </div>
...
const BarcodeScanner = ({ onScan, onClose }) => {
    // FIX: Using Ref to prevent double-initialization
    ...
    useEffect(() => {
        ...
        return () => {
            if (scannerRef.current) {
                try { scannerRef.current.stop().then(() => scannerRef.current.clear()); } catch(e) { console.log(e); }
            }
        };
    }, [onScan]);
    ...
    return (
        <div className="fixed inset-0 bg-black/90 z-[100] flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-sm bg-white rounded-xl overflow-hidden p-4">
                 <h3 className="text-center font-bold mb-2">Scan Roll</h3>
...
const LabelPrint = ({ data, onClose }) => {
  ...
  useEffect(() => {
      if (data && canvasRef.current) {
          try { JsBarcode(canvasRef.current, data.product_id, { format: "CODE128", width: 2.5, height: barcodeScale, displayValue: false, margin: 0 }); } catch (e) { console.error(e); }
      }
  }, [data, barcodeScale]);
      ...
      <div className="bg-white p-4 rounded-xl max-w-lg w-full flex flex-col items-center animate-in fade-in zoom-in-95 my-auto">
        ...
        <div id="print-wrapper" className="flex items-center justify-center bg-gray-200 p-6 rounded-lg border border-dashed border-gray-400 overflow-auto max-h-[70vh] w-full">
            <div id="print-area" style={printStyle}>
                ...
                <div style={rowStyle}><div style={colStyle}><div style={labelInputStyle}>Quality:</div><div style={valueInputStyle}>{data.quality}</div></div><div style={{...colStyle, justifyContent: 'flex-start', flex: 0.6}}><div style={{...labelInputStyle, minWidth: '40px'}}>GSM:</div><div style={valueInputStyle}>{data.gsm}</div></div></div>
                <div style={rowStyle}><div style={colStyle}><div style={labelInputStyle}>Color:</div><div style={valueInputStyle}>{data.color}</div></div><div style={{...colStyle, justifyContent: 'flex-start', flex: 0.6}}><div style={{...labelInputStyle, minWidth: '60px'}}>Width:</div><div style={valueInputStyle}>{data.width_inches}"</div></div></div>
                <div style={rowStyle}><div style={colStyle}><div style={labelInputStyle}>Length:</div><div style={valueInputStyle}>{data.length_meters}m</div></div><div style={{...colStyle, justifyContent: 'flex-start', flex: 0.6}}><div style={{...labelInputStyle, minWidth: '70px'}}>Net Wt:</div><div style={valueInputStyle}>{data.net_weight}kg</div></div></div>
                <div style={{ marginTop: '4px', marginBottom: '8px', display: 'flex', gap: '4px' }}><div style={{...labelInputStyle, width: 'auto'}}>Gross Wt:</div><div style={valueInputStyle}>{data.gross_weight}kg</div></div>
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
            <div className="bg-white p-6 rounded-lg w-full max-w-lg">
                <div className="flex justify-between mb-4"><h2 className="font-bold text-lg">Edit Roll {roll.product_id}</h2><button onClick={onClose}><X size={24} /></button></div>
                ...
                {!isGuest && (
                    <div className="flex flex-col gap-2">
                        {roll.status === 'dispatched' && <button onClick={handleReturnToStock} className="bg-orange-100 text-orange-700 p-3 rounded font-bold flex items-center justify-center gap-2"><RotateCcw size={18}/> Return to Stock</button>}
...
// --- FIX: UPDATED HISTORY VIEW (With Return Button) ---
const HistoryView = ({ rolls = [], onSelectRoll, onExport, onUpdateRoll }) => {
    ...
    // Handler for returning to stock from history list
    const handleReturn = async (roll) => {
        if(window.confirm(`Return ${roll.product_id} to stock?`)) {
            await onUpdateRoll({ ...roll, status: 'in_stock', dispatched_at: null });
        }
    };

    return (
        <div className="space-y-4 h-full flex flex-col">
            <div className="bg-slate-800 text-white p-5 rounded-xl shadow-md relative">
                <button onClick={() => onExport(history)} className="absolute top-4 right-4 bg-white/10 p-2 rounded-lg text-white hover:bg-white/20"><Download size={18}/></button>
                ...
                <div className="flex justify-between items-end"><div><div className="text-3xl font-black">{history.length}</div><div className="text-xs text-slate-400">Rolls Sent</div></div><div className="text-right"><div className="text-3xl font-black text-green-400">{totalDispatchedWeight.toLocaleString()}</div><div className="text-xs text-slate-400">Total Kg</div></div></div>
            </div>
            <div className="flex-1 overflow-y-auto pb-20">
                {history.map(r => (
                    <div key={r.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-3 relative flex justify-between items-center hover:bg-gray-50">
                        <div onClick={() => onSelectRoll(r)} className="cursor-pointer flex-1">
                            <div className="text-xs font-bold text-gray-400 mb-1">{new Date(r.dispatched_at).toLocaleDateString()}</div>
                            <div className="mb-1"><span className="font-mono font-bold text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded">{r.product_id}</span></div>
                            <div className="text-sm font-bold text-gray-900">{r.customer_name || 'Unknown'}</div>
                            <div className="text-xs text-gray-500">{r.net_weight} kg • {r.quality}</div>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); handleReturn(r); }} className="p-3 bg-orange-50 text-orange-600 rounded-lg hover:bg-orange-100 border border-orange-100 ml-2" title="Return to Stock">
                            <RotateCcw size={18}/>
                        </button>
                    </div>
                ))}
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6 animate-in fade-in duration-500">
        {loading ? ( <div className="flex justify-center p-12"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-blue-600"></div></div> ) : (
            <>
                ...
                {/* Ensure handleEditRoll is passed as onUpdateRoll so DispatchView can restore items */}
                ...
                {/* Updated HistoryView with onUpdateRoll for restoration */}
