...
export default function App() {
  ...
  // --- REINFORCED SYNC LOGIC (Handles Dispatch Updates Better) ---
  const syncOfflineData = useCallback(async () => {
    if (!navigator.onLine || loading) return;
    
    // Refresh the pending list from Dexie
    ...
    try {
      for (const roll of pending) {
        // We ensure synced: 1 is sent to cloud so it matches cloud schema
        const dataToSync = { ...roll, synced: 1 };
        // Clean up the local Dexie ID if it exists to avoid UUID conflict
        if (dataToSync.id && typeof dataToSync.id === 'number') delete dataToSync.id;

        const { error } = await supabase
          .from('rolls')
          .upsert(dataToSync, { onConflict: 'product_id' });
        
        if (!error) {
          // Handshake SUCCESS: Mark local as 1
          await db.rolls.update(roll.product_id, { synced: 1 });
        } else {
          console.error(`Sync failed for ${roll.product_id}:`, error.message);
        }
      }
    } catch (err) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (allRemoteData.length > 0) {
        // Any roll from cloud is marked synced: 1 locally
