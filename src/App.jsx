...
// NO EXTERNAL UI LIBRARIES (Prevents all import crashes)
    ...
    if (this.state.hasError) {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-red-50 p-6 text-center font-sans">
          <h1 className="text-2xl font-bold text-red-600 mb-4">⚠️ App Paused</h1>
          <p className="mb-6 text-gray-700">A critical error occurred. Please reset.</p>
          <pre className="text-xs text-left bg-white p-2 border mb-4 max-w-sm overflow-auto">
  ...
  <header className="bg-white border-b border-gray-200 sticky top-0 z-50 shadow-sm safe-area-inset-top">
    <div className="flex justify-between items-center px-4 py-2 max-w-7xl mx-auto h-16">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={onLogout} className="p-2 text-gray-600 hover:bg-red-50 hover:text-red-600 rounded-lg transition-colors">🚪</button>
      </div>
    </div>
    <nav className="flex gap-1 px-2 overflow-x-auto no-scrollbar max-w-7xl mx-auto border-t border-gray-100 md:border-none">
      {[{ id: 'dashboard', icon: '📊', label: 'Dash' }, !isGuest && { id: 'entry', icon: '➕', label: 'New' }, { id: 'stock', icon: '📦', label: 'Stock' }, { id: 'dispatch', icon: '🚚', label: 'Dispatch' }, { id: 'history', icon: '🕒', label: 'History' }, { id: 'materials', icon: '🧱', label: 'Materials' }].filter(Boolean).map(tab => (
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/95 z-[200] flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white p-8 rounded-2xl max-w-md w-full text-center shadow-2xl relative">
                ...
                <button disabled={!name} onClick={() => onSave(name)} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold text-lg disabled:opacity-50">Save Device Name</button>
            </div>
        </div>
    );
};
...
const LabelPrint = ({ data, onClose }) => {
  const savedDesign = safeJSONParse('ksf_label_design_v16', {});
  const savedLabels = safeJSONParse('ksf_label_labels_v16', {});
  ...
  useEffect(() => {
      localStorage.setItem('ksf_label_design_v16', JSON.stringify({ showLogo, labelSize, baseFontSize, logoSize, barcodeScale }));
      const labelsOnly = { 
      ...
      localStorage.setItem('ksf_label_labels_v16', JSON.stringify(labelsOnly));
...
const DispatchView = ({ rolls, isGuest, deviceName, onDispatch, onUpdateRoll }) => {
    ...
    const [sessionList, setSessionList] = useState(() => safeJSONParse('ksf_dispatch_list_v16', []));
    const [customerName, setCustomerName] = useState(() => localStorage.getItem('ksf_dispatch_customer_v16') || '');
    const [vehicleNo, setVehicleNo] = useState(() => localStorage.getItem('ksf_dispatch_vehicle_v16') || '');
    useEffect(() => {
        localStorage.setItem('ksf_dispatch_list_v16', JSON.stringify(sessionList));
        localStorage.setItem('ksf_dispatch_customer_v16', customerName);
        localStorage.setItem('ksf_dispatch_vehicle_v16', vehicleNo);
...
export default function App() {
  ...
  if (!user && !isGuest) { return (<div className="h-[100dvh] flex items-center justify-center bg-slate-50 p-6"><div className="bg-white p-8 rounded-2xl shadow-xl max-w-sm w-full text-center border border-gray-100"><img src="/logo.png" className="h-24 w-auto mx-auto mb-8 object-contain" alt="KSF" /><h1 className="text-2xl font-bold mb-2 text-gray-900">KSF Inventory</h1><p className="text-gray-500 mb-8">Manage your factory floor efficiently.</p><button onClick={handleLogin} className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-bold mb-3 hover:bg-blue-700 active:scale-95 transition-all shadow-lg shadow-blue-200">Login with Google</button><button onClick={handleGuestEntry} className="w-full bg-white text-gray-700 py-3.5 rounded-xl font-bold hover:bg-gray-50 transition border border-gray-200 flex items-center justify-center gap-2">👀 Monitor Only (Guest)</button></div></div>); }
