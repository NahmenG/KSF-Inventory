...
// SAFE ICON IMPORTS (No 'Plus' to prevent crash)
import { 
  ...
  Settings, Minus, RotateCcw, ZoomIn, ChevronUp 
} from 'lucide-react';
...
const LabelPrint = ({ data, onClose }) => {
  const savedDesign = safeJSONParse('ksf_label_design_v14', {});
  const savedLabels = safeJSONParse('ksf_label_labels_v14', {});
  ...
  useEffect(() => {
      localStorage.setItem('ksf_label_design_v14', JSON.stringify({ showLogo, labelSize, baseFontSize, logoSize, barcodeScale }));
      const labelsOnly = { 
      ...
      localStorage.setItem('ksf_label_labels_v14', JSON.stringify(labelsOnly));
...
const NewProductView = ({ formData, setFormData, onSubmit }) => (
    <div className="bg-white p-4 rounded-lg shadow border mt-2">
      {/* NO PLUS ICON - USED FileText INSTEAD */}
...
const DispatchView = ({ rolls, isGuest, deviceName, onDispatch, onUpdateRoll }) => {
    ...
    const [sessionList, setSessionList] = useState(() => safeJSONParse('ksf_dispatch_list_v14', []));
    const [customerName, setCustomerName] = useState(() => localStorage.getItem('ksf_dispatch_customer_v14') || '');
    const [vehicleNo, setVehicleNo] = useState(() => localStorage.getItem('ksf_dispatch_vehicle_v14') || '');
    useEffect(() => {
        localStorage.setItem('ksf_dispatch_list_v14', JSON.stringify(sessionList));
        localStorage.setItem('ksf_dispatch_customer_v14', customerName);
        localStorage.setItem('ksf_dispatch_vehicle_v14', vehicleNo);
