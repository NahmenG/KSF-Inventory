      ...
      <div className="flex items-center gap-3">
        <div className="text-right hidden md:block">
            {isGuest ? (
                ...
                <><div className="text-xs font-bold text-gray-900">{deviceName}</div><div className="text-[10px] text-gray-500">{user?.email}</div></>
            )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4">
            <div className="bg-white p-5 rounded-xl max-w-lg w-full animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                    <div className="col-span-2"><label className="text-xs font-bold text-gray-500 uppercase">Customer</label><input disabled={isGuest} className="w-full border p-2.5 rounded-lg bg-gray-50" value={editData.customer_name || ''} onChange={e => setEditData({...editData, customer_name: e.target.value})} /></div>
...
const StockView = ({ rolls, onPrint, onExport, onSelectRoll }) => {
  ...
  const filtered = rolls.filter(r => {
      ...
      const matchGSM = filters.gsm === 'All' || r.gsm == filters.gsm; // loose equality for string/number
          ...
          <div className="sticky top-0 bg-slate-50 z-10 pb-2 space-y-2">
              <div className="flex gap-2">
                <div className="relative flex-1">
                    ...
                    <input className="w-full pl-10 p-2.5 bg-white border-gray-200 border rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} />
                </div>
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar">
                  <select className="bg-white border p-2 rounded-lg text-xs font-bold text-gray-600 min-w-[100px]" value={filters.color} onChange={e => setFilters({...filters, color: e.target.value})}>
                      {uniqueColors.map(c => <option key={c} value={c}>Color: {c}</option>)}
                  </select>
                  <select className="bg-white border p-2 rounded-lg text-xs font-bold text-gray-600 min-w-[100px]" value={filters.quality} onChange={e => setFilters({...filters, quality: e.target.value})}>
                      {uniqueQualities.map(q => <option key={q} value={q}>Qual: {q}</option>)}
                  </select>
                  <select className="bg-white border p-2 rounded-lg text-xs font-bold text-gray-600 min-w-[80px]" value={filters.gsm} onChange={e => setFilters({...filters, gsm: e.target.value})}>
                      {uniqueGSM.map(g => <option key={g} value={g}>GSM: {g}</option>)}
                  </select>
              </div>
          </div>
...
const HistoryView = ({ rolls, onSelectRoll, onExport }) => {
    ...
    return (
        <div className="space-y-4 h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto pb-20">
                {history.map(r => (
                    <div key={r.id} onClick={() => onSelectRoll(r)} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-3 relative cursor-pointer hover:bg-gray-50">
...
export default function App() {
  ...
  const handleExport = (data = rolls) => {
    const ws = XLSX.utils.json_to_sheet(data);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Sheet1");
    XLSX.writeFile(wb, "KSF_Data.xlsx");
  };
