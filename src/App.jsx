...
export default function App() {
  ...
  // --- EMERGENCY DATA RESTORATION LOGIC (634 IDENTIFIED ROLLS) ---
  const runGhostRollFix = async () => {
    // COMPLETE LIST OF 634 IDENTIFIED GHOST IDs
    ...
    if (!confirm(`Restore ${ghostIds.length} rolls to History? This will fix the 40T discrepancy.`)) return;
    
    setLoading(true);
    try {
      // Supabase update: move rolls back to dispatched state
      const { error } = await supabase
        .from('rolls')
        .update({ 
          status: 'dispatched',
          dispatched_at: '2026-03-21T10:00:00+00:00' 
        })
              ...
              <div className={`p-5 rounded-2xl border transition-all duration-300 ${isAdmin ? 'bg-green-50 border-green-100 shadow-inner' : 'bg-red-50 border-red-100 shadow-sm'}`}>
                ...
                {!isAdmin ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <button onClick={runGhostRollFix} disabled={loading} className="w-full py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase flex items-center justify-center gap-2 hover:bg-blue-700 transition-all"><Database size={14} /> Restore 40T History (Emergency)</button>
