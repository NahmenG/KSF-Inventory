...
// IMPORT ONLY THE SAFEST ICONS
import { 
  Package, Truck, Layers, LogOut, Printer, Search, 
  Download, Database, Calendar, Clock, 
  Trash2, X, Camera, Smartphone, Activity, Eye, FileText, CheckCircle
} from 'lucide-react';
    ...
    <nav className="flex gap-1 px-2 overflow-x-auto no-scrollbar max-w-7xl mx-auto border-t border-gray-100 md:border-none">
      {[{ id: 'dashboard', icon: Database, label: 'Dash' }, !isGuest && { id: 'entry', icon: Plus, label: 'New' }, { id: 'stock', icon: Package, label: 'Stock' }, { id: 'dispatch', icon: Truck, label: 'Dispatch' }, { id: 'history', icon: Calendar, label: 'History' }, { id: 'materials', icon: Layers, label: 'Materials' }].filter(Boolean).map(tab => (
        <button key={tab.id} onClick={() => setTab(tab.id)} className={`flex flex-col md:flex-row items-center justify-center gap-1 md:gap-2 px-3 py-3 md:py-3 rounded-t-lg text-[10px] md:text-sm font-medium transition-all min-w-[65px] flex-1 md:flex-none border-b-2 ${activeTab === tab.id ? 'border-blue-600 text-blue-600 bg-blue-50/50' : 'border-transparent text-gray-500 hover:text-gray-900 hover:bg-gray-50'}`}>
          {/* Fallback to simple text if icon fails, but using safe imports */}
          {tab.id === 'entry' ? <span className="text-lg font-bold">+</span> : <tab.icon size={18} className={activeTab === tab.id ? "text-blue-600" : "text-gray-400"} />}
          <span>{tab.label}</span>
        </button>
      ))}
    </nav>
...
// --- LABEL PRINT COMPONENT (V7 Key - Fresh Start) ---
const LabelPrint = ({ data, onClose }) => {
  const savedDesign = safeJSONParse('ksf_label_design_v7', {});
  const savedLabels = safeJSONParse('ksf_label_labels_v7', {});
  ...
  useEffect(() => {
      ...
      localStorage.setItem('ksf_label_design_v7', JSON.stringify(designSettings));
      localStorage.setItem('ksf_label_labels_v7', JSON.stringify(labelSettings));
...
const DispatchView = ({ rolls, isGuest, deviceName, onDispatch, onUpdateRoll }) => {
    ...
    // SAFE PARSE with V6 Key
    const [sessionList, setSessionList] = useState(() => safeJSONParse('ksf_dispatch_list_v6', []));
    const [customerName, setCustomerName] = useState(() => localStorage.getItem('ksf_dispatch_customer_v6') || '');
    const [vehicleNo, setVehicleNo] = useState(() => localStorage.getItem('ksf_dispatch_vehicle_v6') || '');

    useEffect(() => {
        localStorage.setItem('ksf_dispatch_list_v6', JSON.stringify(sessionList));
        localStorage.setItem('ksf_dispatch_customer_v6', customerName);
        localStorage.setItem('ksf_dispatch_vehicle_v6', vehicleNo);
