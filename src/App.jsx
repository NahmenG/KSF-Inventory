    ...
    <div className="space-y-4 h-full flex flex-col relative pb-20 px-1">
      ...
      <div className="flex-1 overflow-y-auto pb-24 px-1">
        {filtered.length === 0 ? (<div className="text-center text-gray-400 mt-10 italic">No matching rolls found...</div>) : filtered.map(r => (
          <div key={r.id || r.product_id} onClick={() => onSelectRoll(r)} className={`bg-white p-4 rounded-xl border mb-2 shadow-sm flex justify-between items-center cursor-pointer active:scale-[0.98] transition-all ${r.isOffline ? 'border-l-4 border-l-yellow-400' : 'border-gray-100'}`}>
            <div>
    ...
    <div className="space-y-4 pb-24">
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-sm max-h-[90vh] overflow-y-auto shadow-2xl border">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6 text-sm">
              <div className="col-span-2"><label className="text-[10px] font-bold text-gray-500 uppercase">Buyer</label><input className="w-full border p-2 rounded bg-gray-50 outline-none focus:ring-2 focus:ring-blue-100" value={reviewData.customer_name} onChange={e => setReviewData({ ...reviewData, customer_name: e.target.value })} /></div>
              <div><label className="text-[10px] font-bold text-gray-500 uppercase text-blue-600">Net Weight (kg)</label><input type="number" className="w-full border-2 border-green-500 p-2 rounded font-bold outline-none" value={reviewData.net_weight} onChange={e => setReviewData({ ...reviewData, net_weight: e.target.value })} /></div>
              <div><label className="text-[10px] font-bold text-gray-500 uppercase">Quality</label><select className="w-full border p-2 rounded text-xs outline-none" value={reviewData.quality} onChange={e => setReviewData({ ...reviewData, quality: e.target.value })}>{QUALITIES.map(q => <option key={q} value={q}>{q}</option>)}</select></div>
              <div><label className="text-[10px] font-bold text-gray-500 uppercase">GSM</label><input type="number" className="w-full border p-2 rounded text-xs outline-none" value={reviewData.gsm} onChange={e => setReviewData({ ...reviewData, gsm: e.target.value })} /></div>
              <div><label className="text-[10px] font-bold text-gray-500 uppercase">Width</label><input type="number" className="w-full border p-2 rounded text-xs outline-none" value={reviewData.width_inches} onChange={e => setReviewData({ ...reviewData, width_inches: e.target.value })} /></div>
            </div>
      ...
      {sessionList.length > 0 && (
        <div className="bg-white rounded-xl shadow border overflow-hidden border-gray-100">
          ...
          <div className="p-4 bg-gray-50 border-t"><button onClick={() => { if(generateChallanExcel(sessionList, { buyer: customerName, vehicle: vehicleNo, device: deviceName })) { if(confirm("Clear current manifest?")) { setSessionList([]); setCustomerName(''); setVehicleNo(''); } } }} className="w-full bg-green-700 text-white py-4 rounded-xl font-bold flex justify-center gap-2 items-center hover:bg-green-800 shadow-lg active:scale-95 transition-all"><FileSpreadsheet size={20} /> Generate Gate Pass</button></div>
        </div>
      )}
    </div>
  );
...
// --- HISTORY VIEW (FULLY RESTORED Logic) ---
    ...
    <div className="pb-24 px-1">
      ...
      {history.length === 0 ? <div className="text-center text-gray-400 mt-10 italic">No records found...</div> : history.map(r => (
        <div key={r.id} onClick={() => onSelectRoll(r)} className="bg-white p-3 rounded-xl border border-gray-100 mb-2 text-sm shadow-sm hover:border-blue-200 cursor-pointer transition-all">
          ...
          <div className="flex justify-between text-gray-500 text-[10px] tracking-tight uppercase"><span>{r.product_id} • {r.quality}</span><span>{new Date(r.dispatched_at).toLocaleDateString()}</span></div>
        </div>
      ))}
      </div>
    </div>
